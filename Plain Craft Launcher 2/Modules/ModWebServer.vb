...
#Region "网页登录回调"
    ...
    Public Function StartOAuthWaitingCallback(serviceName As String, url As String, completeCallback As OAuthComplete, Optional redirectTemplate As String = Nothing) As Boolean
        ...
        RunInNewThread(
            Sub()
                ...
                SyncLock _webServers
                    ...
                    Dim cbPath As String = "/callback"
                    Dim cbAltPath As String = Nothing
                    If Not String.IsNullOrWhiteSpace(redirectTemplate) Then
                        Try
                            Dim t = redirectTemplate.Replace("{port}", port.ToString())
                            Dim u As New Uri(t)
                            cbPath = u.AbsolutePath.TrimEnd("/"c)
                            If cbPath = String.Empty Then cbPath = "/"
                            '如果模板路径以 /static/ 形式存在，兼容带斜杠路径
                            If cbPath.EndsWith("/static") Then cbAltPath = "/static/"
                        Catch
                            cbPath = "/callback"
                        End Try
                    End If
                    server.Route(cbPath,
                        Function(path, request)
                            ...
                            Return redirect
                        End Function)
                    If Not String.IsNullOrWhiteSpace(cbAltPath) Then
                        server.Route(cbAltPath,
                            Function(path, request)
                                Dim parameterMap As New Dictionary(Of String, String)
                                Dim query = request.Url.Query
                                Dim queryIndex = query.IndexOf("?"c)
                ...
                End SyncLock
                '打开 OAuth URL
                Dim resolved = If(String.IsNullOrWhiteSpace(redirectTemplate), $"http://localhost:{port}/callback", redirectTemplate.Replace("{port}", port.ToString()))
                OpenWebsite(url.Replace("%r", resolved))
